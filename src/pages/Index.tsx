...
import bannerHeroSecurity from "@/assets/banner-hero-security-v2.jpg";
import bannerSecurityHero from "@/assets/banner-security-hero.png";
import bannerGamesSection from "@/assets/banner-games-v2.jpg";
    ...
    const fetchListings = async () => {
      ...
      if (!error && data && data.length > 0) {
        ...
        const mapped: Listing[] = data.map((row) => {
          ...
          return {
            id: row.id, sellerId: row.seller_id, sellerName: s?.name || "Vendedor",
            sellerRating: s?.rating ?? 4.8,
            sellerSales: s?.sales ?? 0, platform: row.category, title: row.title,
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                ...
                <Link to="/marketplace?type=games" className="block rounded-2xl overflow-hidden relative">
                  ...
                  <div className="absolute inset-0 flex items-center">
                    <div className="px-5 sm:px-8 md:px-12 max-w-sm md:max-w-lg">
                      <h2 className="text-white text-base sm:text-xl md:text-3xl lg:text-4xl font-semibold leading-tight">Contas Gamers</h2>
